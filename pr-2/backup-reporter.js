...
class BackupReporter {
  ...
  async generateReport() {
    ...
    console.log(`Amount of backup files: ${amountOfBackupFiles}`);
    
    if (latestFile && latestDate) {
      console.log(`Latest backup file: ${latestFile}`);
      console.log(`Latest backup datetime: ${latestDate.toLocaleString()}`);
    }

    console.log("\nStudents grouped by ID (total amount across all backups):");
    console.log(JSON.stringify(studentsById, null, 2));

    console.log(`\nAverage amount of students in all files: ${averageAmount.toFixed(2)}`);
  }
}
